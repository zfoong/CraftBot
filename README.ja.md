...
| `--enable-gui` | GUIモードを有効化（実験的機能）。画面操作タスクのためにエージェントがGUIモードに切り替えることを許可します。この設定は再起動後も保持されます |
| `--no-gui` | GUIモードを無効化（デフォルト）。エージェントはCLI専用モードで実行され、GUIモードに切り替えることができません。この設定は再起動後も保持されます。OmniParserも自動的に無効化されます |
