  ...
  const icons: Record<string, React.ReactNode> = {
    ...
    jira: (
      <svg width={size} height={size} viewBox="0 0 24 24" fill="#2684FF">
        <path d="M11.571 11.513H0a5.218 5.218 0 0 0 5.232 5.215h2.13v2.057A5.215 5.215 0 0 0 12.594 24V12.518a1.005 1.005 0 0 0-1.023-1.005z"/>
        <path d="M8.613 8.476H21.18a5.218 5.218 0 0 1-5.232 5.215h-2.13v2.057A5.215 5.215 0 0 1 8.59.533v11.948a1.005 1.005 0 0 0 1.023 1.005z" opacity=".65"/>
        <path d="M5.648 5.44h12.566a5.218 5.218 0 0 1-5.232 5.214h-2.13v2.058A5.216 5.216 0 0 1 5.625 18.5V6.445a1.005 1.005 0 0 1 1.023-1.005z" opacity=".35"/>
      </svg>
    ),
    github: (
      <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
...
function IntegrationsSettings() {
  ...
  const [githubWatchTag, setGithubWatchTag] = useState('')
  const [githubWatchRepos, setGithubWatchRepos] = useState('')
  ...
  useEffect(() => {
    ...
    const cleanups = [
      ...
      onMessage('github_settings', (data: unknown) => {
        const d = data as { success: boolean; watch_tag?: string; watch_repos?: string[] }
        if (d.success) {
          setGithubWatchTag(d.watch_tag || '')
          setGithubWatchRepos((d.watch_repos || []).join(', '))
          setGithubSettingsLoaded(true)
        }
      }),
      onMessage('github_settings_result', (data: unknown) => {
        const d = data as { success: boolean; watch_tag?: string; watch_repos?: string[]; message?: string; error?: string }
        setGithubSaving(false)
        if (d.success) {
          setGithubWatchTag(d.watch_tag || '')
          setGithubWatchRepos((d.watch_repos || []).join(', '))
          showToast('success', d.message || 'GitHub settings saved')
        } else {
          showToast('error', d.error || 'Failed to save GitHub settings')
        }
      }),
  ...
  const handleSaveGithubSettings = () => {
    ...
    const reposArray = githubWatchRepos.split(',').map(r => r.trim()).filter(Boolean)
    send('github_update_settings', {
      watch_tag: githubWatchTag,
      watch_repos: reposArray,
    })
  }
  ...
  const handleOpenManage = (integration: Integration) => {
    ...
    if (integration.id === 'github') {
      setGithubSettingsLoaded(false)
      send('github_get_settings')
    }
                    ...
                    <div className={styles.connectForm}>
                      <div className={styles.formGroup}>
                        ...
                        <input
                          ...
                          className={styles.formInput}
                          placeholder="e.g. @craftbot"
                          value={githubWatchTag}
                          onChange={(e) => setGithubWatchTag(e.target.value)}
                        />
                        <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>
                      ...
                      <div className={styles.formGroup}>
                        <label className={styles.formLabel}>Watch Repos</label>
                        <input
                          type="text"
                          className={styles.formInput}
                          placeholder="e.g. owner/repo, myorg/project"
                          value={githubWatchRepos}
                          onChange={(e) => setGithubWatchRepos(e.target.value)}
                        />
                        <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>
